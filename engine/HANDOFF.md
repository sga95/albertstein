...
- Pagine: `index.html` (home con "Latest lab notes" e "Now playing"); `cv/`; `lab/` con `template.html` e `postmortem-template.html`; `progress/` (livelli, missioni, boss con loot, binari, incidenti); `quests/`; `readiness/` (generata); `certs/`; `puzzle/`, `reading/`, `codex/` (nel menu quando hanno contenuto); `404.html`.
...
- `data/progress.json`: `tiers` (6), `missions` (20), `tracks` (shield 9, mind 8, voice 10, hire 8), `incidents`, `quests`. **Lo edita solo Alberto.** Gli strumenti lo leggono, mai lo scrivono.
...
- Binari: `SHIELD.md` (9 passi, per primo), `MIND.md` (8, l'AI come strumento), `VOICE.md` (10), `HIRE.md` (8). Il binario `PI.md` arriva con HANDOFF-3.
...
- `certs.yaml` (catalogo) + `certs-status.json` (requested, granted), `quests.yaml`, `loot.yaml`, `puzzles.yaml`.
...
- Rivedere `data/loot.yaml` e `data/certs.yaml` (costi e scelte). Scrivere il primo puzzle in `data/puzzles.yaml`.
